...
class pbs_job():
    def __init__(self,hpc,jobid,job_type,make_command='lee',\
                 procs_arg=0,node_arg=0,device_arg=None):
        ...
        if (job_type == 'namd') or (job_type=='namdgpu') or (job_type=='sop') \
           or (job_type == 'sopnucleo') or (job_type == 'gsop'):
            ...
            # choose template
            self.dir_templates = os.path.join(my_library,'templates')
            self.dir_job_template = os.path.join(self.dir_templates,job_type)

            
            # if (job_type == 'namd') or (job_type=='sop') or (job_type == 'sopnucleo'):
            #     # self.fn_template = 'job.pbs'
            #     # self.fp_template = os.path.join(self.dir_templates,'%s/template.pbs' \
            #     #                                 % job_type)
            #     self.fp_template = os.path.join(self.cwd,'job.pbs')
                
            # elif (job_type == 'namdgpu'):

            #     # self.fn_template = 'job.pbs'
            #     # self.fp_template = os.path.join(self.dir_templates,'%s/template_%s.pbs' \
            #     #                              % (job_type,self.hpc))
            #     self.fp_template = os.path.join(self.cwd,'job.pbs')
                
            # elif (job_type == 'gsop'):
            #     # self.fn_template = 'meta.sh'
            #     # self.fp_template = os.path.join(self.dir_templates,'%s/template_meta.sh' \
            #     #                              % job_type)
                
            #     self.fn_equil = 'equil.sop'
            #     self.fp_equil = os.path.join(self.dir_job_template,self.fn_equil)
            #     self.fn_pull  = 'pull.sop'
            #     self.fp_pull = os.path.join(self.dir_job_template,self.fn_pull)
            #     self.fp_template = os.path.join(self.cwd,'meta.sh')
            ...
            # template becomes job.pbs | meta.sh in make_template
            # formerly pbs_script | job_script
            if (job_type == 'gsop'):
                self.fn_job_script = 'meta.sh'
                # self.fp_job_script = os.path.join(self.cwd,'meta.sh')
                self.fn_job_equil = 'equil.sop'
                self.fp_job_equil = os.path.join(self.job_path_local,'equil.sop')
                self.fn_job_pull = 'pull.sop'
                self.fp_job_pull = os.path.join(self.job_path_local,'pull.sop')

                self.fp_local_job_script = os.path.join(self.job_path_local,'meta.sh')
            else:
                self.fn_job_script = 'job.pbs'
                # self.fp_job_script = os.path.join(self.cwd,'job.pbs')
                self.fp_local_job_script = os.path.join(self.job_path_local,'job.pbs')
    ...
    def make_template(self):
        ...
        if not os.path.exists(self.job_path_local):
            os.makedirs(self.job_path_local)
        
        if (self.job_type == 'namd') or (self.job_type == 'sop') or \
           (self.job_type == 'sopnucleo'):
            # cp_file(self.dir_job_template,'template.pbs',self.cwd,self.fn_job_script)
            cp_file(self.dir_job_template,'template.pbs',self.job_path_local,self.fn_job_script)
        elif (self.job_type == 'namdgpu'):
            # cp_file(self.dir_job_template,'template_%s.pbs' % self.hpc, \
            #         self.cwd,self.fn_job_script)
            cp_file(self.dir_job_template,'template_%s.pbs' % self.hpc, \
                    self.job_path_local,self.fn_job_script)
        elif (self.job_type == 'gsop'):
            cp_file(self.dir_job_template,'template_meta.sh',self.job_path_local,self.fn_job_script)
            cp_file(self.dir_job_template,self.fn_job_pull,self.job_path_local,self.fn_job_pull)
            cp_file(self.dir_job_template,self.fn_job_equil,self.job_path_local,self.fn_job_equil)

            # # self.fn_job_pull
            # reg_ex(self.cwd,self.fn_job_pull,'xx_jobname_xx',self.jobid)
            # reg_ex(self.cwd,self.fn_job_pull,'xxuserxx',self.user)
            # reg_ex(self.cwd,self.fn_job_pull,'xxmy_dirxx',self.cwd)
            # reg_ex(self.cwd,self.fn_job_pull,'xxjob_dir_namexx',self.job_dir_name)
            # reg_ex(self.cwd,self.fn_job_pull,'xxjob_path_localxx',self.job_path_local)
            # reg_ex(self.cwd,self.fn_job_pull,'xxjob_path_runxx',self.job_path_run)
            # reg_ex(self.cwd,self.fn_job_pull,'xxcompleted_dirxx',self.completed_dir)
            # reg_ex(self.cwd,self.fn_job_pull,'xxsubnamexx',self.job_type)

            # # self.fn_job_equil
            # reg_ex(self.cwd,self.fn_job_equil,'xx_jobname_xx',self.jobid)
            # reg_ex(self.cwd,self.fn_job_equil,'xxuserxx',self.user)
            # reg_ex(self.cwd,self.fn_job_equil,'xxmy_dirxx',self.cwd)
            # reg_ex(self.cwd,self.fn_job_equil,'xxjob_dir_namexx',self.job_dir_name)
            # reg_ex(self.cwd,self.fn_job_equil,'xxjob_path_localxx',self.job_path_local)
            # reg_ex(self.cwd,self.fn_job_equil,'xxjob_path_runxx',self.job_path_run)
            # reg_ex(self.cwd,self.fn_job_equil,'xxcompleted_dirxx',self.completed_dir)
            # reg_ex(self.cwd,self.fn_job_equil,'xxsubnamexx',self.job_type)

        # self.fn_job_script
        reg_ex(self.job_path_local,self.fn_job_script,'xx_jobname_xx',self.jobid)
        reg_ex(self.job_path_local,self.fn_job_script,'xxuserxx',self.user)
        reg_ex(self.job_path_local,self.fn_job_script,'xxmy_dirxx',self.cwd)
        reg_ex(self.job_path_local,self.fn_job_script,'xxjob_dir_namexx',self.job_dir_name)
        reg_ex(self.job_path_local,self.fn_job_script,'xxjob_path_localxx',self.job_path_local)
        reg_ex(self.job_path_local,self.fn_job_script,'xxjob_path_runxx',self.job_path_run)
        reg_ex(self.job_path_local,self.fn_job_script,'xxcompleted_dirxx',self.completed_dir)
        reg_ex(self.job_path_local,self.fn_job_script,'xxsubnamexx',self.job_type)



    def get_nodes_procs(self,node_dct):
        ...
        if self.procs_arg != None:
            ...
            reg_ex(self.job_path_local,'job.pbs','xxprocsxx',str(self.procs_arg))
        if self.node_arg != None:
            # reg_ex(self.cwd,'job.pbs','xxnodexx',self.node_arg)
            reg_ex(self.job_path_local,'job.pbs','xxnodexx',self.node_arg)
        ...
        if self.job_type == 'namdgpu':
            if self.hpc == 'lib':
                ...
                reg_ex(self.job_path_local,'job.pbs','xxnodexx','gpu')
            ...
            if self.hpc == 'lib':
                ...
                # reg_ex(self.cwd,'job.pbs','xxnodexx','gpu') # unneeded ?!
                reg_ex(self.job_path_local,'job.pbs','xxprocsxx',str(max_number_procs))
                reg_ex(self.job_path_local,'job.pbs','xxnodexx','gpu') # unneeded ?!
            elif self.hpc == 'bio':
                try:
                ...
                reg_ex(self.job_path_local,'job.pbs','xxprocsxx',str(max_number_procs))
                reg_ex(self.job_path_local,'job.pbs','xxnodexx',min(node_list))
            ...
            if int(self.nprocs) <= int(max_number_procs):
                reg_ex(self.job_path_local,'job.pbs','xxprocsxx',str(self.nprocs))
            else:
            ...
            node  = min(node_list)
            max_number_procs = node_dct[node]
            reg_ex(self.job_path_local,'job.pbs','xxnodexx',node)
    ...
    def prepare_pbs(self,command_dct={}):
        ...
        if (self.job_type=='sop') or (self.job_type=='sopnucleo'):
            ...
            try:
                reg_ex(self.job_path_local,'job.pbs','xxstructxx',command_dct['sec'])
            except:
                pass
            reg_ex(self.job_path_local,'job.pbs','xxpdbxx',command_dct['pdb'])
            reg_ex(self.job_path_local,'job.pbs','xxcontactxx',command_dct['con'])
            reg_ex(self.job_path_local,'job.pbs','xxseedxx',str(self.seed300))
            reg_ex(self.job_path_local,'job.pbs','xxrunxx',command_dct['run'])
            reg_ex(self.job_path_local,'job.pbs','xxargv_onexx',self.make_command)
...
#  'upd': 'update_dihedrals_dcd_fullGo.c'}
            ...
            reg_ex(self.job_path_local,'job.pbs','xxnamdconfigxx',command_dct['namd'])
            # print 'the value of self.devices is ',self.devices
            reg_ex(self.job_path_local,'job.pbs','xxdevicesxx',self.devices)
    ...
    def construct_local_dir(self,command_dct={}):
        ...
        if (self.job_type=='namd') or (self.job_type=='namdgpu'):
            ...
            cp_file(self.cwd,command_dct['sec'],self.job_path_local,command_dct['sec'])
        ...
        # return max_number_procs
            ...
            reg_ex(self.job_path_local,'equil.sop','xxseedxx',str(seed_equilibrate))
            ...
            reg_ex(self.job_path_local,'pull.sop','xxseedxx',str(self.seed10_7))
    ...
    def submit(self):
        ...
        pipe=subprocess.Popen(['qsub',self.fp_local_job_script],stdin=subprocess.PIPE,
                                stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
        stdout,stderr = pipe.communicate()
        print 'stdout >> ',stdout
        print 'stderr >> ',stderr
