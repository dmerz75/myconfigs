    ...
    def __init__(self,hpc,jobid,job_type,make_command='lee',\
        ...
        if (job_type == 'namd') or (job_type=='namdgpu') or (job_type=='sop') \
           or (job_type == 'sopnucleo') or (job_type == 'gsop'):
            ...
            # elif job_type == 'gsop':
            #     if device_arg == None:
            #         print 'consider resubmitting with -d 0 or -d 1 device argument ...'
            #         time.sleep(4)
            #         self.devices = '0'
            #     else:
            #         self.devices = device_arg
            ...
            # project name, description, time, date
            ...
            # gsop regular expressions' variables set:
            # if not desc == '': self.description = desc

            if (job_type == 'gsop'):

                self.devices = raw_input("Please enter the 'devices':[0]. > ")
                if not self.devices:
                    self.devices = '0'

                self.gsopname = raw_input("Please enter the 'gsopname':[2KHO]. > ")
                if not self.gsopname:
                    self.gsopname = '2KHO'

                self.fixedbead = raw_input("Please enter the 'fixedbead':[0]. > ")
                if not self.fixedbead:
                    self.fixedbead = '0'

                self.pulledbead = raw_input("Please enter the 'pulledbead':[381,220,215]. > ")
                if not self.pulledbead:
                    self.pulledbead = '381'

                self.numsteps = raw_input("Please enter the 'numsteps':[3500000000]. \n \
                use 2,000,000,000 for 2khosbd > ")
                if not self.numsteps:
                    self.numsteps = '3500000000'
    ...
    def make_template(self):
        ...
        if (self.job_type == 'namd') or (self.job_type == 'sop') or \
           (self.job_type == 'sopnucleo'):
            ...
            if self.hpc != '105':
                cp_file(self.dir_job_template,'template_meta_general.sh',self.job_path_local,self.fn_job_script)
                cp_file(self.dir_job_template,'template_pull_general.sop',self.job_path_local,self.fn_job_pull)
                cp_file(self.dir_job_template,'template_equil_general.sop',self.job_path_local,self.fn_job_equil)
            else:
                cp_file(self.dir_job_template,'template_meta_105.sh',self.job_path_local,self.fn_job_script)
                cp_file(self.dir_job_template,'template_pull_105.sop',self.job_path_local,self.fn_job_pull)
                cp_file(self.dir_job_template,'template_equil_105.sop',self.job_path_local,self.fn_job_equil)

            # gsop regular expressions
            for job in [self.fn_job_equil,self.fn_job_pull]:
                reg_ex(self.job_path_local,job,'xxgsopdevicexx',self.devices)
                reg_ex(self.job_path_local,job,'xxgsopnamexx',self.gsopname)
                reg_ex(self.job_path_local,job,'xxfixedbeadxx',self.fixedbead)
                reg_ex(self.job_path_local,job,'xxpulledbeadxx',self.pulledbead)
                reg_ex(self.job_path_local,job,'xxnumstepsxx',self.numsteps)
