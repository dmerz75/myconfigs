...
class MoleculeUniverse:
    ...
    def get_bond_distance(self,step=1,resid_first=0,resid_last=None):
        ...
        atom_1 = self.u.selectAtoms(sel_1)
        atom_2 = self.u.selectAtoms(sel_2)
    ...
    def center_of_mass(self,dct_domains,step=1):
        ...
        with open(os.path.join(self.destdir,'traj_%s_%s.dat' % (self.idn,dct_domains['name'])),'w') as fp:
            for ts in u.trajectory[::step]:
                ...
                sel_nterm = u.selectAtoms("protein and resid %d:%d" % (dct_domains['nterm'][0],dct_domains['nterm'][1]))
                sel_cterm = u.selectAtoms("protein and resid %d:%d" % (dct_domains['cterm'][0],dct_domains['cterm'][1]))
    ...
    def write_coords(self,resid_first,resid_last,start=0,stop=-1,step=1,sel=None):
        ...
        if sel != None:
            ...
            # print selection,tag
            # sys.exit()
        system = self.u.selectAtoms(selection)

        # system = self.u.selectAtoms(selection)
        # print system
        # system = self.u.selectAtoms("all")
        # print system
        # system = self.u.selectAtoms("bynum %d:%d" % (resid_first,resid_last))
        # print system
        # sys.exit()

        for ts in self.u.trajectory[start:stop:step]:
            coord_file = "interim_coord/interim_f%s_r-%s.pdb" % (str(ts.frame),tag)
            # system = self.u.selectAtoms(selection) # 1:384 sopnucleo-adp
            # system = ts.
            print system,len(system),selection
            # sys.exit()
            # if re.search("70.psf",self.psf) != None:
            #     print system
            #     system = system.selectAtoms("protein")
            #     print system
            #     print "using 70.psf and EXITING IMMEDIATELY"
            #     sys.exit()
            if not os.path.exists(coord_file):
                print 'NUMATOMS:',type(ts),ts.numatoms
                print 'ts._pos[0,-1]:','\n',ts._pos[0],'\n',ts._pos[-1]
                print 'writing',ts.frame,'of',len(self.u.trajectory)
                system.write(coord_file)


    def write_coords_(self,resid_first,resid_last,start=0,stop=-1,step=1,sel=None):
        ''' Write out Coord/interim_0.pdb
        (need to make this a kwargs function)
        '''
        # resid_first,resid_last
        ...
        for ts in self.u.trajectory[start:stop:step]:
            coord_file = "interim_coord/interimf%sr%s.pdb" % (str(ts.frame),tag)
            # system = self.u.selectAtoms(selection) # 1:384 sopnucleo-adp
            # system = ts.
            print system,len(system),selection
            # sys.exit()
    ...
    def print_resids(self,start=0,stop=-1,step=1):
        ...
        # sel = self.u.selectAtoms("resid %d:%d" % (start,stop))
        # print sel.segname
        # print sel.resids
        # print sel.resid
        # print sel.resname
        ...
        for i in range(start,stop+1,step):
            ...
            #     # print type(ts),ts.numatoms
            #     # print 'ts._pos[0,-1]:','\n',ts._pos[0],'\n',ts._pos[-1]
            #     resid = self.u.selectAtoms("resid %d" % i)

            #     # segname = self.u.selectAtoms("resid %d" % i).segname
            #     print "%s" % (resid)
